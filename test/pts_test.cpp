...
int grsvd_test(MPI_Comm &comm, int R_d){
	int N_d_sugg = 1000;
	//int R_d = 25;
	// Set up some trees
	const pvfmm::Kernel<double>* kernel=&helm_kernel_xlow;
	const pvfmm::Kernel<double>* kernel_conj=&helm_kernel_conj_xlow;
	pvfmm::BoundaryType bndry=pvfmm::FreeSpace;
	int size, rank;
...
int save_mat_test(MPI_Comm &comm){
	...
	for(int i=0;i<N_d;i++){
		if(i!=0) eye.Set(i-1,0,El::Complex<double>(0.0));
	...
	auto G_lambda  = [&G_matrix,&alpha,&beta] (const El::DistMatrix<El::Complex<double>,El::VC,El::STAR> &x, El::DistMatrix<El::Complex<double>,El::VC,El::STAR> &y) -> void { El::Gemv(El::NORMAL,alpha,G_matrix,x,y) ; return;};
	auto Gs_lambda = [&Gs_matrix,&alpha,&beta](const El::DistMatrix<El::Complex<double>,El::VC,El::STAR> &x, El::DistMatrix<El::Complex<double>,El::VC,El::STAR> &y) -> void { El::Gemv(El::NORMAL,alpha,Gs_matrix,x,y); return;};

	int mult_order = isotree::ParticleTree::mult_order;

	El::Write(G_matrix,SAVE_DIR_STR+"G_matrix"+"_"+std::to_string((long long)(mult_order)),El::ASCII_MATLAB);
	El::Write(Gs_matrix,SAVE_DIR_STR+"Gs_matrix"+"_"+std::to_string((long long)(mult_order)),El::ASCII_MATLAB);
	...
	rsvd::rsvd(U_G,S_G,V_G,G_lambda,Gs_lambda,ctrl_G);
	R_d = ctrl_G.r;
	if(!rank) std::cout << "R_d = " << R_d << std::endl;
...
int main(int argc, char* argv[]){
	...
	int R_d = El::Input("-R_d","Reduced rank",25);
	...
	grsvd_test(comm,R_d);  MPI_Barrier(comm);
